...
const TorrentDetailsDialog = observer(({ dialogStore }: TorrentDetailsDialogProps) => {
  ...
  // Resizable columns
  const defaultPeerWidths = useMemo(() => ({ ip: 100, client: 90, pct: 38, dl: 70, ul: 70, flags: 55 }), []);
  const defaultTrackerWidths = useMemo(() => ({ url: 160, tier: 40, seeds: 50, peers: 50, status: 80 }), []);
